...
RULE_INFO: dict[str, RuleMeta] = {
    ...
    "절단 실패(다음 화를 안 봐도 됨)": RuleMeta(
        "절단은 덧붙이는 장치가 아니라 끊는 위치입니다. 긴장이 이미 풀린 뒤에 멈추면 "
        "독자는 여기서 그만둬도 아쉽지 않고, 그것이 연중의 시작입니다. 마지막 문장을 "
        "고쳐서 될 일이 아닙니다.",
        "더 앞에서 끊으세요. 결과가 나오기 직전 — 행동이 진행 중이거나, 답이 나오기 "
        "전이거나, 문이 열리는 순간에 멈추고, 그 결과는 다음 화의 첫 장면으로 넘기세요.",
        "칼을 맞은 사내가 쓰러졌다. 마커스는 규정집을 덮고 청사를 나섰다.",
        "칼이 눈앞에서 멈추지 않았다."),
