...
def _beats(entities=(), *, forward_pressure=True) -> BeatSheet:
    """Plants a seed by default: an episode that opens nothing is itself a
    finding now, and every other test here is about something else."""
    from novel_agent.artifacts import PlannedSeed

    return BeatSheet(
        episode_number=3, opening_hook="훅", the_one_progression="진전",
        beats=[Beat(text="b", beat_type=BeatType.SETUP)],
        seeds_to_plant=([PlannedSeed(proposed_seed_id="p1", description="미해결 실마리")]
                        if forward_pressure else []),
...
# ── forward pressure: curiosity comes from the flow, not the last line ───────
def test_an_episode_that_opens_nothing_is_flagged():
    """Author feedback: readers continue because of the story's flow, not a
    device on the final line. Measured — 1화 planted 0 떡밥 and ended with the
    protagonist moving on, so no ending could have rescued it."""
    from novel_agent.continuity import RULE_NO_FORWARD_PRESSURE

    vs = deterministic_findings(_draft("본문"), _beats(forward_pressure=False), Canon())
    v = next(x for x in vs if x.rule == RULE_NO_FORWARD_PRESSURE)
    assert v.severity == "major"


def test_planting_a_seed_satisfies_forward_pressure():
    from novel_agent.continuity import RULE_NO_FORWARD_PRESSURE

    vs = deterministic_findings(_draft("본문"), _beats(), Canon())
    assert RULE_NO_FORWARD_PRESSURE not in rules(vs)


def test_a_cliffhanger_beat_also_satisfies_it():
    """A 떡밥 is not the only way to leave pressure open."""
    from novel_agent.artifacts import Beat, BeatSheet, BeatType
    from novel_agent.continuity import RULE_NO_FORWARD_PRESSURE

    bs = BeatSheet(episode_number=3, opening_hook="훅", the_one_progression="진전",
                   beats=[Beat(text="b", beat_type=BeatType.CLIFFHANGER)],
                   closing_cliffhanger="절단")
    assert RULE_NO_FORWARD_PRESSURE not in rules(
        deterministic_findings(_draft("본문"), bs, Canon()))


def test_it_is_a_gate_criterion_not_advisory():
    from novel_agent.reviser import _STRUCTURAL_RULES
    from novel_agent.continuity import RULE_NO_FORWARD_PRESSURE
    assert RULE_NO_FORWARD_PRESSURE in _STRUCTURAL_RULES


def test_the_rule_explains_itself():
    from novel_agent.continuity import RULE_NO_FORWARD_PRESSURE
    from novel_agent.style import rule_meta
    m = rule_meta(RULE_NO_FORWARD_PRESSURE)
    assert m and m.why and m.fix and m.bad and m.good
