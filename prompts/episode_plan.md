...
위 [계획된 떡밥] 중 이 화에서 던지는 것이 자연스러운 게 있으면 `seeds_to_plant`에 넣고 `planned_thread_id`에 대괄호 안의 ID를 그대로 적으세요. 회수되는 부가 한참 뒤라도 실은 미리 물에 들어가 있어야 합니다 — 다만 한 화에 여러 개를 몰아 던지지는 마세요.

계획에 없는 떡밥을 이 화에서 새로 던져도 됩니다. 그 경우 `planned_thread_id`는 비워 두세요. major는 회수하기 전에는 이야기를 끝낼 수 없는 줄기(정체·배신·숨은 목적)이고, minor는 나중에 한 번 되짚으면 끝나는 디테일입니다. due_by_ep에는 몇 화까지 회수할 작정인지 적으세요 — 총 화수를 보고 정하세요. major는 이야기의 뼈대라 보통 후반부에 가서야 회수됩니다. 매 화 심을 필요는 없습니다 — 없으면 빈 목록으로 두세요. 회수할 자신이 없는 떡밥은 심지 않는 편이 낫고, [페이싱 지시]가 새 떡밥을 금지하면 그쪽을 따르세요.
